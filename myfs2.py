...
class DbFS(Operations):
    ...
    def __init__(self):
        ...
        try: 
            ...
            c.execute("""
                ...
                (path,parent,mode,size,ctime,mtime,atime,uid,gid,nlink)
                values (?,?,?,?,?,?,?,?,?,?)
    ...
    def create(self, path, mode):
        ...
        c.execute("""
            ...
            (path,parent,mode,size,ctime,mtime,atime,uid,gid,nlink)
            values (?,?,?,?,?,?,?,?,?,?)
        """, (path, os.path.dirname(path), (S_IFREG | mode), 0, now, now, now, 0, 0, 1))
    ...
    def getattr(self, path, fh=None):
        ...
        fetch = c.fetchone()
        if fetch is not None:
            attr = dict(st_mode=fetch[2], st_size=fetch[3], st_ctime=fetch[4], st_mtime=fetch[5], 
                        st_atime=fetch[6], st_uid=fetch[7], st_gid=fetch[8], st_nlink=fetch[9])
        else:
    ...
    def mkdir(self, path, mode):
        ...
        c.execute("""
            ...
            (path,parent,mode,size,ctime,mtime,atime,uid,gid,nlink)
            values (?,?,?,?,?,?,?,?,?,?)
        """, (path, parent_dir, (S_IFDIR | mode), 0, now, now, now, 0, 0, 2))

        # c.execute("update files set nlink=nlink+1 where path='/'")
        c.execute("update files set nlink=nlink+1 where path=?", (parent_dir,))
    ...
    def readdir(self, path, fh):
        ...
        c.execute("select path from files where path!='/' and parent=?", (path,))
        fetch = c.fetchall()
        if fetch is not None:
            dirlist = ['.', '..'] + [os.path.basename(x[0]) for x in fetch]
        else:
            dirlist = ['.', '..']
    ...
    def symlink(self, target, source):
        ...
        c.execute("""
            ...
            (path,parent,mode,size,ctime,mtime,atime,uid,gid,nlink)
            values (?,?,?,?,?,?,?,?,?,?)
        """, (target, os.path.dirname(target), (S_IFLNK | 0777), len(source), now, now, now, 0, 0, 1))
        c.execute("""
